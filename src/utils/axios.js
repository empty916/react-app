...
import { serverUrl } from '@client/common/config';
...
const createRequestRecords = requestNo => ({
	requestNo,
	timestamp: Date.now(),
});
const requestCache = {
	timeLimit: 666, // 前端接口请求防重复点击阀值
};
...
instance.interceptors.request.use(
	requestConfig => {
		// 请求之前，搞些事情
		const { loading = true, data, url } = requestConfig;
